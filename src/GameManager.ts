...
export class GameManager {
	...
	addHandler({ socket, username, id }: User) {
		socket.addEventListener('message', (messageEvent) => {
			...
			if (message.type === INIT) {
				if (this.pendingUser) {
					const game = new Game({ socket, username, id }, this.pendingUser);
					...
					console.log('yesss still pending');
	...
	removeGame(socket: WebSocket) {
		...
		// remove the game
		this.games = this.games.filter((game) => game.player1.socket !== socket && game?.player2.socket !== socket);
	}
}
