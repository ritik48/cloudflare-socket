...
export class Game {
	...
	constructor(player1: User, player2: User) {
		...
		console.log('player 1 = ', this.player1.socket.readyState);
		console.log('player 2 = ', this.player2.socket.readyState);
	...
	move(socket: WebSocket, direction: Direction) {
		...
		this.board[newPos.x][newPos.y] = socket === this.player1.socket ? this.player1.id : this.player2.id;
		...
		this.player1.socket.send(
			JSON.stringify({
				type: MOVE,
				payload: {
					player: this.player1.socket === socket ? this.player1.id : this.player2.id,
					cur_x: newPos.x,
					cur_y: newPos.y,
					old_x: currentPos.x,
					old_y: currentPos.y,
	...
	isMoveValid({ x, y }: Coords) {
		if (x >= this.row || x < 0 || y >= this.col || y < 0 || this.board[x][y] !== '0') return false;
		return true;
	}
}
